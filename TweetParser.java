...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        if (csvLine != null && 0 <= csvColumn) {
            String[] contents = csvLine.split(",");
            if (csvColumn < contents.length) {
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (li.hasNext()) {
            String line = li.next();
            if (tweetColumn < line.split(",").length && extractColumn(line, tweetColumn) != null) {
                tweets.add(extractColumn(line, tweetColumn));
            }
        }
        return tweets;
    }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        for (String sentence: tweetSplit(removeURLs(tweet))) {
            cleaned.add(parseAndCleanSentence(sentence));
        }
        return cleaned;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String tweet : csvDataToTweets(br, tweetColumn)) {
            for (List<String> sentence: parseAndCleanTweet(tweet)) {
